...
def print_matching_order(match_order):
    ...
    for status in match_order:
        print(status.sid, status.tid, status.name, status.shares, status.price, status.time)
    print()   
...
def print_current_symbol_status():
    ...
    session = Session()
    symbol = 'X'
    all_transaction = session.query(Transaction).filter(Transaction.symbol==symbol)
...
def check_matching_order(session, amount, symbol, limit):
    match_order = session.query(Status).join(Transaction).filter(Status.tid==Transaction.tid)
...
def execute_match_order(session, match_order, current_order_sid):
    current_order = session.query(Status).filter(Status.sid==current_order_sid).first()
    print("In total matching order", len(match_order))
    for order in match_order:
        if abs(order.shares) == abs(current_order.shares):
            match_order_status = session.query(Status).filter(Status.sid==order.sid).first()
...
def execute_order(uid, sym, amt, price):
    ...
    current_order = session.query(Status).filter(Status.tid==current_transaction.tid, Status.name=='open').first()
    
    if current_order is None:
        raise ValueError(
                    "Cannot find current status")
    if current_order.name != 'open':
        print(current_order.sid, current_order.tid, current_order.name, current_order.shares, current_order.price, current_order.time)
        raise ValueError(
                    "Current order is not open")
    match_order = check_matching_order(session, current_transaction.amount, current_transaction.symbol, current_transaction.limit)
